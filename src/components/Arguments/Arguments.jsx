...
const Arguments = () => {
  ...
  return (
    <div className="video-container relative">
      ...
      <div className="modal">
        <p>COME TO VISIT</p>
        <p className=''>BE AMAZED</p>
        <p className=''>WELCOME TO ANGOLA</p>
      </div>
          ...
          <ul className='tableContent'>
              {
                Table.map(({ name, content }) => (
                  <li key={name}>
                    <h2 className='tableTitles'>{name}</h2>
                    <div className=''> 
                          {content.slice(0, visibleItems[name]).map((element, index) => (
                            <p key={index} className='tableOptions'>{element}</p>
                          ))}
                          {content.length > 0 && (
                            <div className=''>
                              {showMore[name] ? (
                                <button onClick={() => handleShowMore(name)} className='tableItems'>
                                ...
                                <button onClick={() => handleShowLess(name)} className='tableItems'>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-6">
                                  <path fillRule="evenodd" d="M12 3.75a.75.75 0 0 1 .75.75v6.75h6.75a.75.75 0 0 1 0 1.5h-6.75v6.75a.75.75 0 0 1-1.5 0v-6.75H4.5a.75.75 0 0 1 0-1.5h6.75V4.5a.75.75 0 0 1 .75-.75Z" clipRule="evenodd" />
                                </svg>
                                <span>See less</span> 
